...
This file provides guidance to Google Antigravity agents when working with code in this repository.

**Note**: For Cursor Agent Mode, see `.cursor/rules/` directory. For Claude Code, see `CLAUDE.md`. This file provides high-level project context for Antigravity agents.
...
## Detailed Rules (Auto-loaded by Antigravity)

The following rule files contain detailed guidelines for specific areas. Antigravity agents should reference these files in `.cursor/rules/`:

- `.cursor/rules/general.md` - Coding standards and conventions
- `.cursor/rules/database.md` - Multi-tenant architecture and account scoping
- `.cursor/rules/security.md` - Security rules and authentication patterns
- `.cursor/rules/testing.md` - Testing requirements and TDD workflow
- `.cursor/rules/forbidden.md` - Protected files that require approval
- `.cursor/rules/autonomous-workflow.md` - Autonomous development patterns

## Workflows

Use workflows in `.agent/workflows/` for common tasks:
- `/test` - Self-healing test loop
- `/fix` - Auto-fix linting and code issues
- `/feature` - TDD feature implementation
